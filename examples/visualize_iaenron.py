...
batch_size = 151  # Lyon=242 # # ia_enron=151 # ia_contacts=113
steps_per_epoch = 5
shuffle = True
verbose = True
...
times_list = torch.linspace(0, 0.8, 100)
...
anim = Animation(embs=embs_pred, time_list=node_times, group_labels=node_ids,
                 color_name="Nodes",
                 #colors=list(map(str, node_ids)),
                 title="Prediction model"+" ".join(filename.split('_')),
                 padding=0.0,
                 dataset=None,
                 # figure_path=f"../outputs/xx4nodes.gif"
                 )
